...
vic_hist_para_ms_6km <- terra::app(vic_hist,
  fun = gev_fit_para_mix_stat,
  cores = 20
)
names(vic_hist_para_ms_6km) <- c("loc", "loctrend", "scale", "shape", "perzero")

writeRaster(vic_hist_para_ms_6km,
            "../data-raw/swe/ACCESS1-0/vic_hist_para_ms_6km.tif",
            overwrite = TRUE
)
...
vic_hist_ms_rtl_6km <- terra::app(vic_hist_para_ms_6km,
  fun = simulate_loads, n = 1000000,
  t = 50, gr = gr_model, cores = 20
)
names(vic_hist_ms_rtl_6km) <- c("RT_I", "RT_II", "RT_III", "RT_IV")
...
################################################################################
vic_hist_ms_rtl_6km <- vic_hist_ms_rtl_6km * 20.8855

writeRaster(vic_hist_ms_rtl_6km,
  "../data-raw/swe/ACCESS1-0/vic_hist_ms_rtl_6km.tif",
  overwrite = TRUE
)
