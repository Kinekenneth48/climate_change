...
#vic_hist_high_res <- terra::aggregate(vic_hist, fact = 5, method = "bilinear")
################################################################################
## STEP 1: Estimate dist parameters
################################################################################


vic_hist_para_ms_6km_smooth <- terra::app(vic_hist,
                                   fun = gev_fit_para_mix_stat,
                                   cores = 20
)
names(vic_hist_para_ms_6km_smooth) <- c("loc", "loctrend", "scale", "shape", "perzero")

vic_hist_para_ms_6km_smooth[["shape"]] = smooth_shape[["shape"]]

writeRaster(vic_hist_para_ms_6km_smooth,
            "../data-raw/swe/ACCESS1-0/vic_hist_para_ms_6km_smooth.tif",
            overwrite = TRUE
)
...
vic_hist_ms_rtl_6km_smooth <- terra::app(vic_hist_para_ms_6km_smooth,
                                  fun = simulate_loads, n = 1000000,
                                  t = 50, gr = gr_model, cores = 20
)
names(vic_hist_ms_rtl_6km_smooth) <- c("RT_I", "RT_II", "RT_III", "RT_IV")
...
################################################################################
vic_hist_ms_rtl_6km_smooth <- vic_hist_ms_rtl_6km_smooth * 20.8855

writeRaster(vic_hist_ms_rtl_6km_smooth,
            "../data-raw/swe/ACCESS1-0/vic_hist_ms_rtl_6km_smooth.tif",
            overwrite = TRUE
)
