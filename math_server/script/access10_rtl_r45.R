...
#comb_vic_r45_high_res <- terra::aggregate(comb_vic_r45, fact = 5, method = "bilinear")
################################################################################
## STEP 1: Estimate dist parameters
################################################################################


vic_r45c_para_ms_6km <- terra::app(comb_vic_r45,
                                    fun = gev_fit_para_mix_stat,
                                    cores = 1
)
names(vic_r45c_para_ms_6km) <- c("loc", "loctrend", "scale", "shape", "perzero")

writeRaster(vic_r45c_para_ms_6km,
            "../data-raw/swe/vic_r45c_para_ms_6km.tif",
            overwrite = TRUE
)
...
vic_r45c_ms_rtl_6km <- terra::app(vic_r45c_para_ms_6km,
                                   fun = simulate_loads, n = 10000,
                                   t = 20, gr = gr_model, cores = 1
)
names(vic_r45c_ms_rtl_6km) <- c("RT_I", "RT_II", "RT_III", "RT_IV")
...
################################################################################
vic_r45c_ms_rtl_6km <- vic_r45c_ms_rtl_6km * 20.8855

writeRaster(vic_r45c_ms_rtl_6km,
            "../data-raw/swe/vic_r45c_ms_rtl_6km.tif",
            overwrite = TRUE
)
