...
hist_gcm_coarser_layers <- terra::rast("D:/data-raw/cmip6/GFDL_ESM4_day_snw_hist.tif")
hist_ua_finer_layers <- terra::rast("D:/data-raw/ua/raster/combined/ua_swe_combined_daily_1982_2014_for_loca_res.tif")

# define the extent of q1
q4_extent <- terra::ext(260, 300, 35, 50)
...
# ============================================================================#
# Crop GCM and UA data for fourth quadrant
# ============================================================================#

hist_gcm_coarser_layers_q4 <- terra::crop(
  x = hist_gcm_coarser_layers,
  y = q4_extent
)

hist_ua_finer_layers_q4 <- terra::crop(
...
future_gcm_coarser_layers_q4 <- hist_gcm_coarser_layers_q4[[11316:12045]]
future_ua_finer_layers_q4 <- hist_ua_finer_layers_q4[[11316:12045]]

# 1982 to 2012 snow year (hist layers)
hist_ua_finer_layers_q4 <- hist_ua_finer_layers_q4[[1:11315]]
hist_gcm_coarser_layers_q4 <- hist_gcm_coarser_layers_q4[[1:11315]]
...
# 9086.72 sec elapsed
tictoc::tic()
fine_to_coarse_hist_ua_layers_q4 <- terra::project(hist_ua_finer_layers_q4,
  hist_gcm_coarser_layers_q4[[1]],
  method = "bilinear"
)
tictoc::toc()
...
# Mask GCM
# ============================================================================#
future_gcm_coarser_layers_q4 <- terra::mask(
  x = future_gcm_coarser_layers_q4,
  mask = fine_to_coarse_hist_ua_layers_q4[[1]]
)

hist_gcm_coarser_layers_q4 <- terra::mask(
  x = hist_gcm_coarser_layers_q4,
  mask = fine_to_coarse_hist_ua_layers_q4[[1]]
)
...
# Save data
# ============================================================================#
terra::writeRaster(hist_gcm_coarser_layers_q4,
  filename = "D:data-raw/macca/q4/hist_gcm_coarser_layers_q4.tif",
  overwrite = TRUE
)


terra::writeRaster(hist_ua_finer_layers_q4,
  filename = "D:data-raw/macca/q4/hist_ua_finer_layers_q4.tif",
  overwrite = TRUE
)

terra::writeRaster(future_gcm_coarser_layers_q4,
  filename = "D:data-raw/macca/q4/future_gcm_coarser_layers_q4.tif",
  overwrite = TRUE
)


terra::writeRaster(future_ua_finer_layers_q4,
  filename = "D:data-raw/macca/q4/future_ua_finer_layers_q4.tif",
  overwrite = TRUE
)


terra::writeRaster(fine_to_coarse_hist_ua_layers_q4,
  filename = "D:data-raw/macca/q4/fine_to_coarse_hist_ua_layers_q4.tif",
  overwrite = TRUE
)


hist_gcm_coarser_layers_q4= rast("data-raw/macca/q4/hist_gcm_coarser_layers_q4.tif")
hist_ua_finer_layers_q4= rast("data-raw/macca/q4/hist_ua_finer_layers_q4.tif")
future_gcm_coarser_layers_q4= rast("data-raw/macca/q4/future_gcm_coarser_layers_q4.tif")
future_ua_finer_layers_q4= rast("data-raw/macca/q4/future_ua_finer_layers_q4.tif")
fine_to_coarse_hist_ua_layers_q4= rast("data-raw/macca/q4/fine_to_coarse_hist_ua_layers_q4.tif")
...
future_dates <- time(future_gcm_coarser_layers_q4)
hist_dates <- time(hist_gcm_coarser_layers_q4)
...
for (i in 1:terra::nlyr(future_gcm_coarser_layers_q4)) {
  ...
  future_moving_dates <- get_21_days_across_years_vector(
    terra::time(future_gcm_coarser_layers_q4[[i]]), min_year_future, 
    max_year_future
  )

  # Compute the mean of the future data within the 21-day period
  future_mean <- mean(future_gcm_coarser_layers_q4[[which(future_dates %in% 
                                                            future_moving_dates)]],
    na.rm = TRUE
  )
  ...
  hist_moving_dates <- get_21_days_across_years_vector(
    terra::time(future_gcm_coarser_layers_q4[[i]]),
    min_year_hist, max_year_hist
  )

  # Compute the mean of the historical data within the calculated 21-day periods
  hist_mean <- mean(hist_gcm_coarser_layers_q4[[which(hist_dates %in% hist_moving_dates)]],
    na.rm = TRUE
  )
...
diff_adjusted_layers_q4 <- rast(diff_adjusted_list)


epoch_future_gcm_coarser_layers_q4 <- future_gcm_coarser_layers_q4 - diff_adjusted_layers_q4
...
terra::writeRaster(epoch_future_gcm_coarser_layers_q4,
  filename = "data-raw/macca/q4/epoch_future_gcm_coarser_layers_q4.tif",
  overwrite = TRUE
)

terra::writeRaster(diff_adjusted_layers_q4,
  filename = "data-raw/macca/q4/diff_adjusted_layers_q4.tif",
  overwrite = TRUE
)
...
# Determine the maximum and minimum years in the historical data
max_year_hist <- max(as.numeric(format(time(fine_to_coarse_hist_ua_layers_q4), "%Y")))
min_year_hist <- min(as.numeric(format(time(fine_to_coarse_hist_ua_layers_q4), "%Y")))

list_hist <- list()

# 17996.2 sec elapsed
tictoc::tic()
for (i in 1:terra::nlyr(fine_to_coarse_hist_ua_layers_q4)) {
  gcm_hist <- hist_gcm_coarser_layers_q4[[i]]
  ...
  obs_array <- as.array(fine_to_coarse_hist_ua_layers_q4[[which(hist_dates 
                                                                %in% hist_moving_dates)]])

  gcm_array <- as.array(hist_gcm_coarser_layers_q4[[which(hist_dates 
                                                          %in% hist_moving_dates)]])
  ...
  ext(corrected_array) <- c(xmin = 260, xmax = 300, ymin = 35, ymax = 50)
  crs(corrected_array) <- crs(hist_gcm_coarser_layers_q4)
  time(corrected_array) <- time(hist_gcm_coarser_layers_q4[[i]])

  list_hist[[i]] <- corrected_array
}
tictoc::toc()

bias_corrected_hist_gcm_q4 <- rast(list_hist)

terra::writeRaster(bias_corrected_hist_gcm_q4,
  filename = "E:data-raw/macca/q4/bias_corrected_hist_gcm_q4.tif",
  overwrite = TRUE
)
...
# 1072.18 sec elapsed
tictoc::tic()
for (i in 1:terra::nlyr(epoch_future_gcm_coarser_layers_q4)) {
  gcm_future <- epoch_future_gcm_coarser_layers_q4[[i]]
  ...
  obs_array <- as.array(fine_to_coarse_hist_ua_layers_q4[[which(hist_dates %in%
    hist_moving_dates)]])

  gcm_array <- as.array(epoch_future_gcm_coarser_layers_q4[[which(time(
    epoch_future_gcm_coarser_layers_q4
  ...
  corrected_array <- rast(corrected_array)
  ext(corrected_array) <- c(xmin = 260, xmax = 300, ymin = 35, ymax = 50)
  crs(corrected_array) <- crs(epoch_future_gcm_coarser_layers_q4)
  time(corrected_array) <- time(epoch_future_gcm_coarser_layers_q4[[i]])

  list_future[[i]] <- corrected_array
}
tictoc::toc()

bias_corrected_future_gcm_q4 <- rast(list_future)


terra::writeRaster(bias_corrected_future_gcm_q4,
  filename = "data-raw/macca/q4/bias_corrected_future_gcm_q4.tif",
  overwrite = TRUE
)
...
tictoc::tic()
for (i in 1:nlyr(bias_corrected_future_gcm_q4)) {
  # ============================================================================#
  # if global = TRUE, return vector of index else layers of index
  # ============================================================================#

  best_analog_index <- analog_day_index(
    future_layer = bias_corrected_future_gcm_q4[[i]],
    hist_layers = fine_to_coarse_hist_ua_layers_q4,
    n = 20, common_dates = time(fine_to_coarse_hist_ua_layers_q4),
    global = TRUE
  )
  ...
  ananlog_finer_layers <- hist_ua_finer_layers_q4[[best_analog_index]]
  #ananlog_coarser_layers <- fine_to_coarse_hist_ua_layers_q4[[best_analog_index]]
...
fine_donwscaled_future_q4 = rast(fine_list)

terra::writeRaster(fine_donwscaled_future_q4,
                   filename = "data-raw/macca/q4/fine_donwscaled_future_q4.tif",
                   overwrite = TRUE
)
