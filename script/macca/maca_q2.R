...
q2_extent <- terra::ext(235, 265, 35, 50)
...
# ============================================================================#
# Crop GCM and UA data for second quadrant
# ============================================================================#

hist_gcm_coarser_layers_q2 <- terra::crop(
  x = hist_gcm_coarser_layers,
  y = q2_extent
)

hist_ua_finer_layers_q2 <- terra::crop(
...
future_gcm_coarser_layers_q2 <- hist_gcm_coarser_layers_q2[[11316:12045]]
future_ua_finer_layers_q2 <- hist_ua_finer_layers_q2[[11316:12045]]

# 1982 to 2012 snow year (hist layers)
hist_ua_finer_layers_q2 <- hist_ua_finer_layers_q2[[1:11315]]
hist_gcm_coarser_layers_q2 <- hist_gcm_coarser_layers_q2[[1:11315]]
...
# 9086.72 sec elapsed
tictoc::tic()
fine_to_coarse_hist_ua_layers_q2 <- terra::project(hist_ua_finer_layers_q2,
  hist_gcm_coarser_layers_q2[[1]],
  method = "bilinear"
)
tictoc::toc()
...
# Mask GCM
# ============================================================================#
future_gcm_coarser_layers_q2 <- terra::mask(
  x = future_gcm_coarser_layers_q2,
  mask = fine_to_coarse_hist_ua_layers_q2[[1]]
)

hist_gcm_coarser_layers_q2 <- terra::mask(
  x = hist_gcm_coarser_layers_q2,
  mask = fine_to_coarse_hist_ua_layers_q2[[1]]
)
...
# Save data
# ============================================================================#
terra::writeRaster(hist_gcm_coarser_layers_q2,
  filename = "D:data-raw/macca/q2/hist_gcm_coarser_layers_q2.tif",
  overwrite = TRUE
)


terra::writeRaster(hist_ua_finer_layers_q2,
  filename = "D:data-raw/macca/q2/hist_ua_finer_layers_q2.tif",
  overwrite = TRUE
)

terra::writeRaster(future_gcm_coarser_layers_q2,
  filename = "D:data-raw/macca/q2/future_gcm_coarser_layers_q2.tif",
  overwrite = TRUE
)


terra::writeRaster(future_ua_finer_layers_q2,
  filename = "D:data-raw/macca/q2/future_ua_finer_layers_q2.tif",
  overwrite = TRUE
)


terra::writeRaster(fine_to_coarse_hist_ua_layers_q2,
  filename = "D:data-raw/macca/q2/fine_to_coarse_hist_ua_layers_q2.tif",
  overwrite = TRUE
)


hist_gcm_coarser_layers_q2= rast("data-raw/macca/q2/hist_gcm_coarser_layers_q2.tif")
hist_ua_finer_layers_q2= rast("data-raw/macca/q2/hist_ua_finer_layers_q2.tif")
future_gcm_coarser_layers_q2= rast("data-raw/macca/q2/future_gcm_coarser_layers_q2.tif")
future_ua_finer_layers_q2= rast("data-raw/macca/q2/future_ua_finer_layers_q2.tif")
fine_to_coarse_hist_ua_layers_q2= rast("data-raw/macca/q2/fine_to_coarse_hist_ua_layers_q2.tif")
...
future_dates <- time(future_gcm_coarser_layers_q2)
hist_dates <- time(hist_gcm_coarser_layers_q2)
...
for (i in 1:terra::nlyr(future_gcm_coarser_layers_q2)) {
  ...
  future_moving_dates <- get_21_days_across_years_vector(
    terra::time(future_gcm_coarser_layers_q2[[i]]), min_year_future,
    max_year_future
  )

  # Compute the mean of the future data within the 21-day period
  future_mean <- mean(future_gcm_coarser_layers_q2[[which(future_dates %in% 
                                                         future_moving_dates)]],
    na.rm = TRUE
  )
...
diff_adjusted_layers_q2 <- rast(diff_adjusted_list)


epoch_future_gcm_coarser_layers_q2 <- future_gcm_coarser_layers_q2 - diff_adjusted_layers_q2
...
terra::writeRaster(epoch_future_gcm_coarser_layers_q2,
  filename = "data-raw/macca/q2/epoch_future_gcm_coarser_layers_q2.tif",
  overwrite = TRUE
)

terra::writeRaster(diff_adjusted_layers_q2,
  filename = "data-raw/macca/q2/diff_adjusted_layers_q2.tif",
  overwrite = TRUE
)
...
# Determine the maximum and minimum years in the historical data
max_year_hist <- max(as.numeric(format(time(fine_to_coarse_hist_ua_layers_q2), "%Y")))
min_year_hist <- min(as.numeric(format(time(fine_to_coarse_hist_ua_layers_q2), "%Y")))

list_hist <- list()

# 17996.2 sec elapsed
tictoc::tic()
for (i in 1:terra::nlyr(fine_to_coarse_hist_ua_layers_q2)) {
  gcm_hist <-  hist_gcm_coarser_layers_q2[[i]]
  ...
  obs_array <- as.array(fine_to_coarse_hist_ua_layers_q2[[which(hist_dates 
                                                                %in% hist_moving_dates)]])

  gcm_array <- as.array(hist_gcm_coarser_layers_q2[[which(hist_dates
                                                          %in% hist_moving_dates)]])
  ...
  ext(corrected_array) <- c(xmin = 235, xmax = 265, ymin = 35, ymax = 50)
  crs(corrected_array) <- crs(hist_gcm_coarser_layers_q2)
  time(corrected_array) <- time(hist_gcm_coarser_layers_q2[[i]])

  list_hist[[i]] <- corrected_array
}
tictoc::toc()

bias_corrected_hist_gcm_q2 <- rast(list_hist)

terra::writeRaster(bias_corrected_hist_gcm_q2,
  filename = "data-raw/macca/q2/bias_corrected_hist_gcm_q2.tif",
  overwrite = TRUE
)
...
# 1072.18 sec elapsed
tictoc::tic()
for (i in 1:terra::nlyr(epoch_future_gcm_coarser_layers_q2)) {
  gcm_future <- epoch_future_gcm_coarser_layers_q2[[i]]
  ...
  obs_array <- as.array(fine_to_coarse_hist_ua_layers_q2[[which(hist_dates %in%
    hist_moving_dates)]])

  gcm_array <- as.array(epoch_future_gcm_coarser_layers_q2[[which(time(
    epoch_future_gcm_coarser_layers_q2
  ...
  corrected_array <- rast(corrected_array)
  ext(corrected_array) <- c(xmin = 235, xmax = 265, ymin = 35, ymax = 50)
  crs(corrected_array) <- crs(epoch_future_gcm_coarser_layers_q2)
  time(corrected_array) <- time(epoch_future_gcm_coarser_layers_q2[[i]])

  list_future[[i]] <- corrected_array
}
tictoc::toc()

bias_corrected_future_gcm_q2 <- rast(list_future)


terra::writeRaster(bias_corrected_future_gcm_q2,
  filename = "data-raw/macca/q2/bias_corrected_future_gcm_q2.tif",
  overwrite = TRUE
)
...
tictoc::tic()
for (i in 1:nlyr(bias_corrected_future_gcm_q2)) {
  # ============================================================================#
  # if global = TRUE, return vector of index else layers of index
  # ============================================================================#

  best_analog_index <- analog_day_index(
    future_layer = bias_corrected_future_gcm_q2[[i]],
    hist_layers = fine_to_coarse_hist_ua_layers_q2,
    n = 20, common_dates = time(fine_to_coarse_hist_ua_layers_q2),
    global = TRUE
  )
  ...
  ananlog_finer_layers <- hist_ua_finer_layers_q2[[best_analog_index]]
  #ananlog_coarser_layers <- fine_to_coarse_hist_ua_layers[[best_analog_index]]
...
fine_donwscaled_future_q2 = rast(fine_list)

terra::writeRaster(fine_donwscaled_future_q2,
                   filename = "data-raw/macca/q2/fine_donwscaled_future_q2.tif",
                   overwrite = TRUE
)
