...
q1_extent <- terra::ext(235, 265, 24, 40)
...
hist_gcm_coarser_layers_q1 <- terra::crop(
  x = hist_gcm_coarser_layers,
  y = q1_extent
)

hist_ua_finer_layers_q1 <- terra::crop(
...
# load data
# ============================================================================#
hist_gcm_coarser_layers_q1 = rast("data-raw/macca/q1/hist_gcm_coarser_layers_q1.tif")
hist_ua_finer_layers_q1 = rast("data-raw/macca/q1/hist_ua_finer_layers_q1.tif")
future_gcm_coarser_layers_q1 = rast("data-raw/macca/q1/future_gcm_coarser_layers_q1.tif")
future_ua_finer_layers_q1 = rast("data-raw/macca/q1/future_ua_finer_layers_q1.tif")
fine_to_coarse_hist_ua_layers_q1 = rast("data-raw/macca/q1/fine_to_coarse_hist_ua_layers_q1.tif")
...
future_dates <- time(future_gcm_coarser_layers_q1)
hist_dates <- time(hist_gcm_coarser_layers_q1)
...
for (i in 1:terra::nlyr(future_gcm_coarser_layers_q1)) {
  ...
  future_moving_dates <- get_21_days_across_years_vector(
    terra::time(future_gcm_coarser_layers_q1[[i]]), min_year_future,
    max_year_future
  )

  # Compute the mean of the future data within the 21-day period
  future_mean <- mean(future_gcm_coarser_layers_q1[[which(future_dates %in% 
                                                            future_moving_dates)]],
    na.rm = TRUE
  )
...
diff_adjusted_layers_q1 <- rast(diff_adjusted_list)


epoch_future_gcm_coarser_layers_q1 <- future_gcm_coarser_layers_q1 -
  diff_adjusted_layers_q1
...
terra::writeRaster(epoch_future_gcm_coarser_layers_q1,
  filename = "data-raw/macca/q1/epoch_future_gcm_coarser_layers_q1.tif",
  overwrite = TRUE
)

terra::writeRaster(diff_adjusted_layers_q1,
  filename = "data-raw/macca/q1/diff_adjusted_layers_q1.tif",
  overwrite = TRUE
)
...
# Determine the maximum and minimum years in the historical data
max_year_hist <- max(as.numeric(format(time(fine_to_coarse_hist_ua_layers_q1), "%Y")))
min_year_hist <- min(as.numeric(format(time(fine_to_coarse_hist_ua_layers_q1), "%Y")))

list_hist <- list()

# 17996.2 sec elapsed
tictoc::tic()
for (i in 1:terra::nlyr(fine_to_coarse_hist_ua_layers_q1)) {
  gcm_hist <-  hist_gcm_coarser_layers_q1[[i]]
  ...
  obs_array <- as.array(fine_to_coarse_hist_ua_layers_q1[[which(hist_dates
                                                    %in% hist_moving_dates)]])

  gcm_array <- as.array(hist_gcm_coarser_layers_q1[[which(hist_dates
                                                          %in% hist_moving_dates)]])
  ...
  ext(corrected_array) <- c(xmin = 235, xmax = 265, ymin = 24, ymax = 40)
  crs(corrected_array) <- crs(hist_gcm_coarser_layers_q1)
  time(corrected_array) <- time(hist_gcm_coarser_layers_q1[[i]])

  list_hist[[i]] <- corrected_array
}
tictoc::toc()

bias_corrected_hist_gcm_q1 <- rast(list_hist)

terra::writeRaster(bias_corrected_hist_gcm_q1,
  filename = "data-raw/macca/q1//bias_corrected_hist_gcm_q1.tif",
  overwrite = TRUE
)
...
# 1072.18 sec elapsed
tictoc::tic()
for (i in 1:terra::nlyr(epoch_future_gcm_coarser_layers_q1)) {
  gcm_future <-  epoch_future_gcm_coarser_layers_q1[[i]]
  ...
  obs_array <- as.array(fine_to_coarse_hist_ua_layers_q1[[which(hist_dates %in%
    hist_moving_dates)]])

  gcm_array <- as.array(epoch_future_gcm_coarser_layers_q1[[which(time(
    epoch_future_gcm_coarser_layers_q1
  ...
  corrected_array <- rast(corrected_array)
  ext(corrected_array) <- c(xmin = 235, xmax = 265, ymin = 24, ymax = 40)
  crs(corrected_array) <- crs(epoch_future_gcm_coarser_layers_q1)
  time(corrected_array) <- time(epoch_future_gcm_coarser_layers_q1[[i]])

  list_future[[i]] <- corrected_array
}
tictoc::toc()

bias_corrected_future_gcm_q1 <- rast(list_future)


terra::writeRaster(bias_corrected_future_gcm_q1,
  filename = "data-raw/macca/q1/bias_corrected_future_gcm_q1.tif",
  overwrite = TRUE
)
...
tictoc::tic()
for (i in 1:nlyr(bias_corrected_future_gcm_q1)) {
  # ============================================================================#
  # if global = TRUE, return vector of index else layers of index
  # ============================================================================#

  best_analog_index <- analog_day_index(
    future_layer = bias_corrected_future_gcm_q1[[i]],
    hist_layers = fine_to_coarse_hist_ua_layers_q1,
    n = 20, common_dates = time(fine_to_coarse_hist_ua_layers_q1),
    global = TRUE
  )
  ...
  ananlog_finer_layers <- hist_ua_finer_layers_q1[[best_analog_index]]
...
fine_downscaled_future_q1 = rast(fine_list)

terra::writeRaster(fine_downscaled_future_q1,
                   filename = "data-raw/macca/q1/fine_downscaled_future_q1.tif",
                   overwrite = TRUE
)
