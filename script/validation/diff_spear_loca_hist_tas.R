...
time_values <- terra::time(spear_hist_tas)
...
spear_hist_tas_filtered <- spear_hist_tas[[filtered_indices]]
...
crs_r_fine_agg <- crs(loca_hist_tas)
crs_spear_hist_tas_filtered <- crs(spear_hist_tas_filtered)

# Compare CRS
if (crs_r_fine_agg != crs_spear_hist_tas_filtered) {
  warning("The CRS of the two SpatRaster do not match.")

  # Transform one raster to match the CRS of the other
  loca_hist_tas <- terra::project(
    x = loca_hist_tas,
    y = crs_spear_hist_tas_filtered
  )
} else {
...
extent_to_match <- terra::ext(spear_hist_tas_filtered)
loca_hist_tas_crop <- terra::crop(loca_hist_tas, extent_to_match)

# Resample the finer raster to match the coarser raster
loca_hist_tas_resampled <- resample(loca_hist_tas_crop, spear_hist_tas_filtered,
  method = "average"
)
...
multiple_per_diff_hist_tas <- ((spear_hist_tas_filtered - loca_hist_tas_resampled) /
  (spear_hist_tas_filtered)) * 100

single_per_diff_hist_tas <- terra::app(multiple_per_diff_hist_tas, fun = mean, 
                                       na.rm = TRUE)
...
# Plot raster
plot(crop(single_per_diff_hist_tas, usa_extent),
  col = colorRampPalette(brewer.pal(9, "RdYlBu"))(length(my_breaks) - 1),
  breaks = my_breaks, main = "Mean Percentage Difference of Average Surface Temperature between historical SPEAR and LOCA",
