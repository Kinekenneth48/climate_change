...
SWER = 0.1
melt_rate = 1.0916 #millimeters per Kelvin
################################################################################
...
for (j in 1:(length(unique_years) - 1)) {
  ...
  # Initialize master_SWE_raster and child_SWE_raster
  master_SWE_raster = terra::setValues(swe_subset[[1]], NA)
  child_SWE_raster = terra::setValues(swe_subset[[1]], NA)
  
  for (i in 1:terra::nlyr(swe_subset)) {
    # Assuming that temp_subset and pr_subset correspond to the current layer
    SWE_changes <- terra::ifel(temp_subset[[i]] < temp_threshold,
                               pr_subset[[i]] * SWER,
                               -(temp_subset[[i]] - temp_threshold) * melt_rate)
    
    cum_swe <- sum(SWE_changes, child_SWE_raster, na.rm = TRUE) 
    child_SWE_raster <- terra::ifel(cum_swe >= 0, cum_swe, 0)
    
    master_SWE_raster <- max(master_SWE_raster, child_SWE_raster, na.rm = TRUE)
  }
  ...
  # from kg m-2 s-1 to mm per day
  d <- max_swe - master_SWE_raster*86400000
