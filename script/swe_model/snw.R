...
hist_gcm_coarser_layers <- terra::rast("E:/data-raw/cmip6/GFDL_ESM4_day_snw_hist.tif")
hist_ua_finer_layers <- terra::rast("E:/data-raw/ua/raster/combined/ua_swe_combined_daily_1982_2014_for_loca_res.tif")
...
future_gcm_coarser_layers <- hist_gcm_coarser_layers[[11316:12045]]
future_ua_finer_layers <- hist_ua_finer_layers[[11316:12045]]

# 1982 to 2012 snow year (hist layers)
hist_ua_finer_layers <- hist_ua_finer_layers[[1:11315]]
hist_gcm_coarser_layers <- hist_gcm_coarser_layers[[1:11315]]
...
# 9086.72 sec elapsed
tictoc::tic()
fine_to_coarse_hist_ua_layers <- terra::project(hist_ua_finer_layers,
  hist_gcm_coarser_layers[[1]],
  method = "bilinear"
)
tictoc::toc()
...
terra::writeRaster(fine_to_coarse_hist_ua_layers,
  filename = "E:data-raw/other/fine_to_coarse_hist_ua_layers.tif",
  overwrite = TRUE
)

fine_to_coarse_hist_ua_layers <-
  terra::rast("E:data-raw/other/fine_to_coarse_hist_ua_layers.tif")
...
future_dates <- time(future_gcm_coarser_layers)
hist_dates <- time(hist_gcm_coarser_layers)
...
for (i in 1:terra::nlyr(future_gcm_coarser_layers)) {
...
diff_adjusted_layers <- rast(diff_adjusted_list)


epoch_future_gcm_coarser_layers <- future_gcm_coarser_layers - diff_adjusted_layers
...
terra::writeRaster(epoch_future_gcm_coarser_layers,
  filename = "E:data-raw/other/epoch_future_gcm_coarser_layers.tif",
  overwrite = TRUE
)

terra::writeRaster(diff_adjusted_layers,
  filename = "E:data-raw/other/diff_adjusted_layers.tif",
  overwrite = TRUE
)
...
# Determine the maximum and minimum years in the historical data
max_year_hist <- max(as.numeric(format(time(fine_to_coarse_hist_ua_layers), "%Y")))
min_year_hist <- min(as.numeric(format(time(fine_to_coarse_hist_ua_layers), "%Y")))

list_hist <- list()

# 17996.2 sec elapsed
tictoc::tic()
for (i in 1:terra::nlyr(fine_to_coarse_hist_ua_layers)) {
  gcm_hist <- terra::mask(
    x = hist_gcm_coarser_layers[[i]],
    mask = fine_to_coarse_hist_ua_layers[[1]]
  )
  ...
  obs_array <- as.array(fine_to_coarse_hist_ua_layers[[which(hist_dates %in% hist_moving_dates)]])

  gcm_array <- as.array(hist_gcm_coarser_layers[[which(hist_dates %in% hist_moving_dates)]])
  ...
  ext(corrected_array) <- c(xmin = 235, xmax = 300, ymin = 24, ymax = 50)
...
bias_corrected_hist_gcm <- rast(list_hist)

terra::writeRaster(bias_corrected_hist_gcm,
  filename = "E:data-raw/other/bias_corrected_hist_gcm.tif",
  overwrite = TRUE
)
...
# 1072.18 sec elapsed
tictoc::tic()
for (i in 1:terra::nlyr(epoch_future_gcm_coarser_layers)) {
  gcm_future <- terra::mask(
    x = epoch_future_gcm_coarser_layers[[i]],
    mask = fine_to_coarse_hist_ua_layers[[1]]
  )
  ...
  obs_array <- as.array(fine_to_coarse_hist_ua_layers[[which(hist_dates %in%
    hist_moving_dates)]])

  gcm_array <- as.array(epoch_future_gcm_coarser_layers[[which(time(
    epoch_future_gcm_coarser_layers
  ...
  corrected_array <- rast(corrected_array)
  ext(corrected_array) <- c(xmin = 235, xmax = 300, ymin = 24, ymax = 50)
  crs(corrected_array) <- crs(epoch_future_gcm_coarser_layers)
  time(corrected_array) <- time(epoch_future_gcm_coarser_layers[[i]])

  list_future[[i]] <- corrected_array
}
tictoc::toc()

bias_corrected_future_gcm <- rast(list_future)


terra::writeRaster(bias_corrected_future_gcm,
  filename = "E:data-raw/other/bias_corrected_future_gcm.tif",
  overwrite = TRUE
)
...
tictoc::tic()
for (i in 1:nlyr(bias_corrected_future_gcm)) {
  # ============================================================================#
  # if global = TRUE, return vector of index else layers of index
  # ============================================================================#

  best_analog_index <- analog_day_index(
    future_layer = bias_corrected_future_gcm[[i]],
    hist_layers = fine_to_coarse_hist_ua_layers,
    n = 20, common_dates = time(fine_to_coarse_hist_ua_layers),
    global = TRUE
  )
  ...
  ananlog_finer_layers <- hist_ua_finer_layers[[best_analog_index]]
  ananlog_coarser_layers <- fine_to_coarse_hist_ua_layers[[best_analog_index]]

  df_x <- as.data.frame(ananlog_coarser_layers)

  y_values <- as.data.frame(bias_corrected_future_gcm[[i]])
  colnames(y_values) <- "y"

  regress_data <- c(df_x, y_values)

  regress_data <- as.data.frame(regress_data)

  colnames(regress_data) <- c(paste("SWE", 1:nlyr(ananlog_coarser_layers), sep = "_"), "y")

  regress_model <- lm(formula = y ~ 0 + ., data = regress_data)
  coefficients <- coef(regress_model)

  for (i in 1:nlyr(ananlog_finer_layers)) {
    ananlog_finer_layers[[i]] <- ananlog_finer_layers[[i]] * coefficients[i] 
  }
