find_top_storm_sums_w_index <- function(data, num_zeros) {
  ...
  for (i in 1:(length(data) - 1)) {
    # Check for a sequence of num_zeros zeros to mark the end of a storm
    if (i < (length(data) - num_zeros + 1) && all(data[i:(i + num_zeros - 1)] == 0, na.rm = TRUE)) {
      # Calculate storm sum, skipping over zero-only segments
      end_index <- i - 1
      ...
      # Update the start index for the next storm
      start_index <- i + num_zeros
    }
  }
  
  # Check and sum the last segment if it doesn't end with zeros
  ...
  top_storms <- cbind(storm_sums[top_storms_indices], start_indices[top_storms_indices], end_indices[top_storms_indices])
  
  # Flatten and replace NA with 0
  top_storms_vector <- as.vector(t(top_storms))
  top_storms_vector[is.na(top_storms_vector)] <- 0
  
  #return storm sum, start index, end index;storm sum, start index, end index (in that order)
  return(top_storms_vector)
}
